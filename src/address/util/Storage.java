...
public class Storage {
	...
	public static List<Music> parser(LinkedList<String> musicData) {
		...
		// using flow operation which is not such a good choice 'cause container's performance is not 
		// so good as we expected
		return musicData.stream().map(path -> {
			File musicFile = new File(path);
			if(!musicFile.exists())
				// Here is better to return a default error object and remove it later
				// if the original mp3 file has been moved away or deleted
				return null;
			byte[] buffer = new byte[128];
			try {
				// using a random access object to get data from the mp3 file with stride defined
				// by ourselves
				RandomAccessFile raf = new RandomAccessFile(musicFile, "r");
				byte[] header = new byte[10];
				raf.read(header);
				// byte array for reading header information with 10 bytes for each
				byte[] buf = new byte[10];
				// declared word segment in a music object
				String musicName = "";
				String album = "";
				String singer = "";
				String ID = "";
				// here is the byte array to store album music if exists
				byte[] image = new byte[1];
				while(true) {
					raf.read(buf);
					// if ends with a TAG, read it
					...
					// print current frame's name for debugging
					System.out.println(new String(buf, 0, 4));
					// following is the size of this frame, check validity firstly
					...
					// content for current frame with size we have got
					byte[] content = new byte[size];
					raf.read(content);
					// get encoding for this frame's data
					String encoding = "";
					switch(content[0]) {
						case 0:
					...
					// convert header information from byte array into string
					String frame_header = new String(buf, 0, 4);
					// get corresponding information according to header of this frame, and attach data to
					// corresponding word seg of current music object as string
					switch(frame_header) {
						case "TXXX":
							ID = new String(content, 6, size - 6, encoding);
							break;
						// header for music's name
						case "TIT2":
							musicName = new String(content, 1, size - 1, encoding);
							break;
						// header for album's name
						case "TALB":
						...
						// album's picture
						case "APIC":
							// write out picture for debugging
							image = content;
